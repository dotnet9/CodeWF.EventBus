...
- 当前版本：`3.4.5.23`，版本号统一维护在根目录 `Directory.Build.props` 的 `<Version>` 节点。
...
同步的 `Publish` / `Query` 仅适用于匹配的处理器全部返回 `void`。如果处理器返回 `Task`，请使用 `PublishAsync` / `QueryAsync`，避免在 UI 同步上下文中产生死锁。
...
在仓库根目录运行 `pack.bat`，脚本会执行 `dotnet restore` 和 Release 构建，并把生成的 NuGet 包输出到 `artifacts\packages`。
